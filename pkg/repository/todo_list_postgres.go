...
func (r *TodoListPostgres) Create(userId int, item todo.TodoList) (int, error) {
	tx, err := r.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (r *TodoListPostgres) GetAll(userId int) ([]todo.TodoList, error) {
	...
	query := fmt.Sprintf("SELECT tl.* FROM %s tl INNER JOIN %s AS ul ON (tl.id = ul.list_id) WHERE ul.user_id = $1",
...
func (r *TodoListPostgres) Delete(userId, listId int) error {
	query := fmt.Sprintf(`DELETE FROM %s tl 
							USING %s ul WHERE tl.id = ul.list_id AND ul.user_id = $1 AND tl.id = $2`,
		todoListsTable, usersListsTable)
